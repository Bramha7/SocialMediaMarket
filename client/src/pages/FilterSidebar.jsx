  ...
  return (
    <div className={`${showFilterPhone ? 'max-sm:fixed' : 'max-sm:hidden'} max-sm:inset-0 z-100 max-sm:h-screen max-sm:overflow-scroll bg-white rounded-lg shadow-sm border border-gray-200 h-fit sticky top-24 md:min-w-[300px]`}>
      <div className="p-4 border-b border-gray-200">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="sm:hidden text-sm border text-gray-700 px-3 py-1 rounded" onClick={() => setShowFilterPhone(false)}>Apply</button>
      ...
      <div className="p-4 space-y-6 sm:max-h-[calc(100vh-200px)] overflow-y-scroll no-scrollbar">
        {/*searchbar*/}
        <div className="flex items-center justify-between">
          <input type="text" placeholder="Search by username, platform, niche" className="w-full text-sm px-3 py-2 border border-gray-300 rounded-md outline-indigo-500" onChange={onChangeSearch} value={search} />
        </div>
        {/*filter setup*/}
        <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          const updated = checked
                            ...
                            : current.filter((p) => p !== platform.value) // fixed typo
...

          {
            expandedSections.price && (
              <div className="space-y-3">
                ...
                <input type="range" min="0" max="200000" step="100" value={filters.maxPrice || 200000} onChange={(e) => onFilterChange({ ...filters, maxPrice: parseInt(e.target.value) })} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-indigo-600" />
                <div className="flex items-center justify-between text-sm text-gray-600">
                  <span>{currency}0</span>
                  <span>{currency}{(filters.maxPrice || 200000).toLocaleString()}</span>
        ...
        <div>
          <button onClick={() => toggleSection("followers")} className="flex items-center justify-between w-full mb-3">
            <label className="text-sm font-medium text-gray-800">Minimum Range</label>
...

          {
            expandedSections.followers && (
              <select className="w-full px-3 py-2 border border-gray-300 rounded-lg text-gray-700 outline-indigo-500" value={filters.minFollowers?.toString() || "0"}
        ...
        <div>
...

          {
            expandedSections.niche && (
              <select className="w-full px-3 py-2 border border-gray-300 rounded-lg text-gray-700 outline-indigo-500" value={filters.niche || ""}
