...
const ManageListing = () => {
  ...
  const niches = ['lifestyle', 'fitness', 'health', 'travel', 'gaming',
  ...
  const handleImageUpload = async (event) => {
    ...
    if (files.length + formData.images.length > 5) return toast.error(" Can add upto 5 images")
    setFormData((prev) => ({
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <Section title={'Basic Information'} >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <InputField label={'Listing Title'} value={formData.title} placeHolder={'eg., Premium helath Instagram Account'} onChange={(v) => handleInputChange('title', v)} required='true' />
              ...
              <SelectField label={'Nich/Category '} options={niches} value={formData.niche} onChange={(v) => handleInputChange('niche', v)} required={true} />
          ...
          <Section title={'Account Metrics'}>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              ...
              <InputField label={'Engagement Rate (%)'} type="number" min={0} value={formData.engagement_rate} placeHolder={'4'} onChange={(v) => handleInputChange('engagement_rate', v)} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              ...
              <SelectField label={'Primary Audience Age Range '} options={ageRanges} value={formData.age_range} onChange={(v) => handleInputChange('age_range', v)} required={true} />
          ...
          <Section title={'Pricing & Description'}>
            ...
            <InputField label={'Asking Price (USD)'} type="number" value={formData.price} placeHolder={'2499.8'} onChange={(v) => handleInputChange('price', v)} required='true' />
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
                {formData.images.map((img, index) => (
                  <div key={index} className=" relative">
                    ...
                    <button className="absolute -top-2.5 -right-2.5 size-6 bg-red-600 text-white rounded-full hover:bg-red-700" type="button" onClick={() => removeImage(index)}>
                      x
...
const InputField = ({ label, value, onChange, placeHolder, type = 'text', required = false, min = null, max = null }) => (
  ...
  >
    ...
    <input type={type} placeholder={placeHolder} min={min} value={value} onChange={(e) => onChange(e.target.value)} className="w-full px-3 py-1.5 text-gray-600 border rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 border-gray-300" max={max} required={required} />
...
const TextAreaField = ({ label, value, onChange, required = false }) => (
  <div>
    <label className="block text-sm font-medium text-gray-700 mb-2">{label} </label>
    <textarea rows={5} value={value} onChange={(e) => onChange(e.target.value)} className="w-full px-3 py-1.5 text-gray-600 border rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 border-gray-300" required={required} />  </div>
)
