...
const MyOrder = () => {
  ...
  const [expandedId, setExpanedId] = useState(null)

  const fetchOrders = async () => {
    try {
      setLoading(true)
      const token = await getToken()
      const { data } = await api.get('/listing/user-orders', {
        headers: {
            ...
            return (
              <div key={id} className="bg-white rounded-lg border border-gray-200 p-5 flex flex-col max-w-4xl">
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <div className="flex gap-2 mt-2">
                          {
                            listing.verified && (
                              <span className="flex items-center text-xs bg-indigo-50 text-indigo-600 px-2 py-1.5 rounded-md">
                                <CheckCircle2 className="w-2 h-3 mr-1" />Verified
                          ...
                          {
                            listing.monetized && (
                              <span className="flex items-center text-xs bg-green-50 text-green-600 px-2 py-1.5 rounded-md">
                ...
                <div className="flex flex-col gap-2 items-end">
                  <button onClick={() => setExpanedId((p) => (p === id ? null : id))} className="flex items-center gap-2 bg-white border border-gray-200 px-3 py-2 rounded hover:shadow text-sm" aria-expanded={isExpaned}>{isExpaned ? (
                    <>
                      ...
                      <div className="space-y-2">
                        {
                          ...
                          credential.updatedCredential.map((cred) => (<div key={cred.name} className="flex items-center justify-between gap-3 bg-gray-50 rounded-md p-2">
                            <div>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              } className="px-2 py-1.5 text-xs bg-white border border-r-gray-200 rounded hover:shadow" title="Copy Credential">
                                <Copy size={4} className="size-4" />
